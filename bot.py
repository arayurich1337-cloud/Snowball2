...
течение 5 минут — это не мгновенный чат, а почтовый ящик (бот живёт в GitHub
...
накопилось больше, чем влезает в один запуск, остаток ждёт в очереди и тоже
приходит личным сообщением — сводок и «разборов пропущенного» больше нет.
Окно свежести: наружу выпускаются только сделки текущего дня и последних
FRESH_HOURS часов; всё, что датировано раньше, уходит в /old, но не в ленту.
...
FRESH_HOURS (окно свежести в часах, по умолчанию 6; 0 = слать всё).
...
MAX_SEEN = 8000           # потолок памяти «что слано» (ключи текущего окна не вытесняются)
...
FRESH_HOURS = 6           # шлём только текущий день и последние N часов (0 = всё)
...
def first_deal_row(html: str) -> str:
    """Отпечаток верхней строки таблицы сделок — по нему видно, сдвинулась ли
    страница при листании (и не уехали ли данные вверх между запросами)."""
    rows = parse_rows(html)
    if not rows:
        return ""
    r = rows[0]
    return "|".join(str(r.get(k, "")) for k in
                    ("operation", "asset", "date", "qty", "price", "amount"))
        ...
        try:
            ...
            for n in range(2, max(1, pages) + 1):
                ...
                for attempt in range(3):
                    # Клик по номеру страницы иногда НЕ сдвигает таблицу: в этот
                    # раз мы получили 300 строк, из них 100 — повтор страницы 1.
                    # Молча листать дальше значило бы потерять хвост списка,
                    # поэтому сверяем первую строку и повторяем клик.
                    try:
                        if not pg.evaluate(CLICK_PAGE_JS, n):
                            log.info("страницы %d нет — дальше не идём", n)
                            break
                        pg.wait_for_timeout(2200 + 900 * attempt)
                        nxt = pg.content()
                    except Exception as e:
                        log.debug("листать дальше не вышло: %s", str(e)[:120])
                        break
                    if first_deal_row(nxt) != first_deal_row(chunks[-1]):
                        chunks.append(nxt)
                        moved = True
                        break
                    log.info("страница %d не сдвинулась, пробую ещё (%d/3)", n, attempt + 1)
                if not moved:
                    break
            return "\n".join(chunks)
        finally:
...
/old — что скрыто как не свежее (дата раньше, чем FRESH_HOURS часов назад)
/again 150 — забыть, что эти строки уже присылались, и прислать заново
...
def status_text(state: dict, name: str, rows: list[dict], url: str) -> str:
    ...
    pend_sell = sum(1 for r in (state.get("pending") or []) if is_sell(r))
    L.append(f"В очереди на отправку: <b>{pend}</b>"
             + ("" if not pend else f" — разойдётся за {max(1, -(-pend // MAX_PER_RUN))} прог.")
             + (f", из них продаж: {pend_sell}" if pend_sell else ""))
    L.append(f"Отпечатков в базе: {len(state.get('seen') or {})}")
    if state.get("old_count"):
        L.append(f"Скрыто как не свежие: {state['old_count']} "
                 f"(дата раньше {fresh_hours()} ч. назад) — /old")
...
def run_cmd(cmd: str, arg: str, state: dict, *, name: str, rows: list[dict], url: str,
    ...
    if cmd == "last":
        ...
        return ("\n".join(out), False)
    if cmd in ("again", "заново"):
        # "/again 150" — только свежие; "/again 150 все" — на этот раз и старьё тоже.
        want_all = any(w in arg.lower() for w in ("все", "всё", "all"))
        num = "".join(ch for ch in (arg or "") if ch.isdigit())
        try:
            n = max(1, min(400, int(num or 150)))
        except ValueError:
            return (f"Не понял число: {esc(arg)}. Пример: /again 150", False)
        pick = rows[:n]
        if not pick:
            return (nodata, False)
        keys = {row_key(r, name) for r in pick}
        state["seen"] = {k: v for k, v in (state.get("seen") or {}).items() if k not in keys}
        state["old_hold"] = []
        if want_all:
            state["fresh_off"] = 1
        return (f"🧹 Забыл {len(keys)} верхних строк: на следующем запуске (ждём ~5 мин) "
                + ("пришлю их заново все, включая старые даты." if want_all else
                   f"пришлю их заново; дата раньше {fresh_hours()} ч. уйдёт в /old, "
                   "а не в ленту.")
                , True)
    if cmd == "old":
        hold = [r for r in (state.get("old_hold") or []) if isinstance(r, dict)]
        if not hold:
            return ("Скрытых «старых» сделок нет — всё, что свежее "
                    f"{fresh_hours() or '∞'} ч., пошло в ленту.", False)
        return ("🗑 <b>Скрыто как не свежее</b> — " + esc(name) + "\n\n"
                + digest_rows(hold, name)
                + f"\n\nНужны в ленту: пришлите /again {min(len(hold) + 40, 400)} все.", False)
    if cmd == "queue":
...
#  Один цикл
# -------------------------------------------------------------------------- #
def is_sell(row: dict) -> bool:
    """Продажа ли строка — по колонке «Операция» (сайт пишет «Продажа»)."""
    return "продаж" in str(row.get("operation", "")).lower()


def fresh_hours() -> int:
    """Размер окна свежести в часах. Переменная FRESH_HOURS, 0 = фильтр выключен."""
    v = os.environ.get("FRESH_HOURS")
    if v is not None and v.strip().lstrip("-").isdigit():
        return max(0, int(v))
    return FRESH_HOURS


def day_start(date_str: str) -> float | None:
    """Начало суток (МСК) для даты «15/09/2026» в секундах эпохи.

    timegm, а не mktime: в Actions локальное время = UTC, и mktime сдвинул бы
    границу на 3 часа, а на другой машине — куда угодно.
    """
    k = _dkey(date_str)
    if len(k) != 8 or not k.isdigit():
        return None
    try:
        return calendar.timegm(time.strptime(k, "%Y%m%d")) - TZ_HOURS * 3600
    except ValueError:
        return None


def is_old(row: dict, hours: int | None = None) -> bool:
    """Не свежая — её сутки закончились раньше, чем «сейчас минус N часов».

    Сайт показывает только дату, без времени, поэтому «15/09» = весь день 15-го:
    вчерашняя сделка живёт в ленте до 06:00 сегодняшнего дня (при N = 6), дальше
    уходит в /old. Ровно то, что просил пользователь: «текущий день и 6 часов до
    этого, больше не надо». Дату не понял — НЕ прячем: лучше лишнее письмо.
    """
    h = fresh_hours() if hours is None else hours
    if not h:
        return False
    start = day_start(str(row.get("date", "")))
    if start is None:
        return False
    return start + 86400 <= time.time() - h * 3600


def copies(v) -> int:
    """Сколько копий отпечатка уже разослано.

    Прежний код писал сюда 0 для строк, которых лишь коснулся, а читал как
    int(v or 1) — ноль превращался в «одну отправил», и сделка замолкала
    навсегда. Теперь 0 означает 0.
    """
...
def clamp_seen(rows: list[dict], seen: dict, portfolio: str) -> None:
    ...
    for k, c in occ.items():
        if k in seen and c < copies(seen[k]):
            seen[k] = c
...
def queue_fresh(rows: list[dict], seen: dict, state: dict, portfolio: str
    ...
    sent = Counter({k: copies(v) for k, v in (seen or {}).items() if copies(v)})
    occ = Counter()
    for r in rows:
...
def run(token: str, chats: list[str], url: str, use_browser: bool, state_path: Path,
    ...
    # Свежесть по ДАТЕ СДЕЛКИ, а не по порядку в таблице: владелец вносит правки
    # задним числом, и без этого фильтра лента забивается августом (см. is_old).
    # /again N «все» разово выключает фильтр на один прогон.
    hours = 0 if state.pop("fresh_off", 0) else fresh_hours()
    old_ones = [r for r in queue if is_old(r, hours)]
    if old_ones:
        queue = [r for r in queue if not is_old(r, hours)]
        for r in old_ones:                      # помечаем: решение сознательное
            k = row_key(r, name)
            state["seen"][k] = copies(state["seen"].get(k)) + 1
        state["old_hold"] = old_ones[:40]
        state["old_count"] = len(old_ones)
    if stale:
    ...
    # Только личные сообщения, в порядке ленты. «📦 Разбор пропущенного» был тем
    # местом, где продажи терялись для человека: 6 продаж 15-го ушли списком, и
    # «ни одной не прислал» было правдой. Лишнее теперь не сворачивается, а ждёт
    # в очереди и приходит личным на следующем прогоне.
    head = queue[:MAX_PER_RUN]
    rest = list(queue[len(head):])
    catchup = len(queue) > CATCHUP_LIMIT
    delivered = skipped = fails = 0
    ...
    # Память «что уже разослано». Раньше здесь держали ровно MAX_SEEN записей и
    # резали по числу: при 3000 ключей каждый прогон вытеснял 5–26 штук, а
    # вытесненная сделка перставала быть «отправленной» и прилетала повторно
    # («сегодня прислал покупку за 10 сентября»). Теперь пустые записи не
    # создаются, а при подрезке ключи текущего окна не вырезаются никогда.
    window = {row_key(r, name) for r in rows}
    seen = {k: copies(v) for k, v in (state["seen"] or {}).items() if copies(v) > 0}
    for r in reversed(rows):                   # свежие — в конец: они не вытеснятся
        k = row_key(r, name)
        if k in seen:
            seen[k] = seen.pop(k)
    if len(seen) > MAX_SEEN:
        cut = len(seen) - MAX_SEEN
        state["seen"] = {k: v for i, (k, v) in enumerate(seen.items())
                         if i >= cut or k in window}
    else:
        state["seen"] = seen
    save_state(state_path, state)
    ...
    if old_ones and int(time.time()) - int(state.get("old_at") or 0) > MISSED_COOLDOWN:
        send(f"🗑 Скрыл {len(old_ones)} сделок: их даты раньше, чем {hours} ч. "
             f"назад (сейчас {stamp()}). В ленту идут только сегодняшний день и "
             f"последние {hours} ч.\nСписок: /old. Показать в ленте один раз: "
             "/again 200 все.")
        state["old_at"] = int(time.time())
        save_state(state_path, state)
    ...
    return 0, delivered, ("queued" if rest else "")
