...
FRESH_DAYS (сколько дней считать сделку свежей, по умолчанию 7; 0 = слать всё).
...
FRESH_DAYS = 7            # сделки СТАРШЕ этого срока в ленту не просим (0 = всегда слать)
...
/old — что бот отложил как «слишком старое» (дата глубже FRESH_DAYS дней)
...
def status_text(state: dict, name: str, rows: list[dict], url: str) -> str:
    ...
    if state.get("old_count"):
        L.append(f"Отложено как не свежие: {state['old_count']} "
                 f"(дата глубже {fresh_days()} дн.) — /old")
    ...
    if cmd in ("again", "заново"):
        ...
        return (f"🧹 Забыл {len(keys)} верхних строк: на следующем запуске (ждём ~5 мин) "
                + ("пришлю их заново все, включая старые даты." if want_all else
                   f"пришлю их заново; дата глубже {fresh_days()} дн. уйдёт в /old, "
                   "а не в ленту.")
                , True)
    if cmd == "old":
        hold = [r for r in (state.get("old_hold") or []) if isinstance(r, dict)]
        if not hold:
            return ("Отложенных «старых» сделок нет — всё, что свежее "
                    f"{fresh_days() or '∞'} дн., пошло в ленту.", False)
        return ("🗑 <b>Отложено как не свежее</b> — " + esc(name) + "\n\n"
...
def fresh_days() -> int:
    """Сколько дней считаем свежей сделкой. Переменная FRESH_DAYS, 0 = фильтр выкл."""
    v = os.environ.get("FRESH_DAYS")
    if v is not None and v.strip().lstrip("-").isdigit():
        return max(0, int(v))
    return FRESH_DAYS


def is_old(row: dict, days: int | None = None) -> bool:
    """Датирована глубже FRESH_DAYS? Такой хлам в ленту не идёт: владелец правит
    портфель задним числом, и без фильтра «разбор пропущенного» превращается в
    перечисление августа — именно на это и жалуются («зачем мне 31.08, сегодня 10-е»).
    """
    d = fresh_days() if days is None else days
    if not d:
        return False
    k = _dkey(str(row.get("date", "")))
    if len(k) != 8 or not k.isdigit():
        return False
    try:
        then = time.mktime(time.strptime(k, "%Y%m%d"))
    except ValueError:
        return False
    return time.time() - then > d * 86400
...
def run(token: str, chats: list[str], url: str, use_browser: bool, state_path: Path,
    ...
    days = 0 if state.pop("fresh_off", 0) else fresh_days()
    old_ones = [r for r in queue if is_old(r, days)]
    if old_ones:
        queue = [r for r in queue if not is_old(r, days)]
        for r in old_ones:                      # помечаем: решение сознательное
            k = row_key(r, name)
            state["seen"][k] = int(state["seen"].get(k, 0) or 0) + 1
        state["old_hold"] = old_ones[:60]
    ...
    if catchup:
        # Догон после ночи/выходных. Продажу человек ждёт сильнее покупки (это
        # выход из позиции), поэтому в ленту лично берутся СНАЧАЛА продажи, а
        # покупки доезжают сводкой. Ничего не теряется: сводка перечисляет всех.
        sells = [r for r in queue if is_sell(r)][:MAX_PER_RUN // 2]
        buys = [r for r in queue if not is_sell(r)][:CATCHUP_LIMIT]
        pos = {id(r): i for i, r in enumerate(queue)}
        head = sorted(sells + buys, key=lambda r: pos[id(r)])
        chosen = {id(r) for r in head}
        rest = [r for r in queue if id(r) not in chosen]
    else:
    ...
    if old_ones and int(time.time()) - int(state.get("old_at") or 0) > MISSED_COOLDOWN:
        send(f"🗑 Отложил {len(old_ones)} сделок как не свежие: дата глубже "
             f"{days} дн. (сегодня {stamp()[:5]}). На сайте они лежат ниже "
             "по списку — это и есть «пришляет старьё».\n"
             "Список: /old. Прислать их же заново в ленту: /again 200.")
